...
export default function JobAdvertisementIsActiveDetail() {
  let { id } = useParams();
  ...
  function handleisActive(id, isActive) {
    jobAdvertisementService.isActive(id, isActive);
  }
  return (
    <div>
      <Table primary style={{ marginLeft: "2em" }} celled unstackable>
        ...
        <Table.Footer>
          <Table.Row>
            <Table.HeaderCell colSpan="11"></Table.HeaderCell>
          </Table.Row>
        </Table.Footer>
      </Table>

      <Button onClick={() => handleisActive(jobAdvertisement.id, true)} primary>
        Make Active
      </Button>
      <Button
        onClick={() => handleisActive(jobAdvertisement.id, false)}
        secondary
      >
        Make Passive
      </Button>
